...
// organise an array of pages by separating out the featured pages
export const sortPages = pages => {
  const featuredPage = pages.filter(page => page.data.featured === true)[0];
  const otherPages = pages.filter(
    page => page.requirePath !== featuredPage.requirePath,
  );
  return {
    featuredPage,
    otherPages,
